...
public class BodyEditorLoader {
     ...
     */
        public void attachFixture(Body body, String name, FixtureDef fd, Vector2 scale, Vector2 origin) {
        RigidBodyModel rbModel = model.rigidBodies.get(name);
        if (rbModel == null) throw new RuntimeException("Name '" + name + "' was not found.");

        //origin = vec.set(rbModel.origin).scl(scale);
    ...
    /**
     * Check if the body with the given name is exists
     */
    public boolean bodyExists(String name){
        return model.rigidBodies.get(name) != null;
    }
